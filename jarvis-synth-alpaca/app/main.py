...
Layer 4 (JARVIS synth)  →  Layer 5 (Risk Guard + Alpaca paper execution)

Trades a mix of crypto (24/7) + US stocks (RTH only) on Alpaca paper.
"""
...
async def profit_lock_heartbeat(
    cfg, executor: AlpacaExecutor, profit_lock: ProfitLock, bot
) -> None:
    """Poll Alpaca NAV; lock profits when threshold crossed; alert + reset baseline."""
    try:
    ...
    # widget reflects this lock. Tagged source="jarvis-synth-alpaca" for
    # attribution alongside the OANDA worker. Failure is silently logged.
    try:
...
async def daily_summary_job(
    cfg, executor: AlpacaExecutor, profit_lock: ProfitLock,
...
async def main_async() -> None:
    ...
    fetcher = AlpacaFetcher(cfg.alpaca_api_key, cfg.alpaca_secret_key)
    ...
    # Initialize the profit-lock ledger using current Alpaca NAV as the
    ...
    online_msg = (
        f"🧠 JARVIS Synth · ALPACA online — full 5-layer pipeline armed.\n"
        f"📊 Trading {len(cfg.instruments)} instruments (crypto + stocks).\n"
