"""Boot-time configuration for the Alpaca paper-trading worker.
...
ALPACA_PAPER_BASE_URL_DEFAULT = "https://paper-api.alpaca.markets"
...
def load_config() -> Config:
    ...
    base_url = os.environ.get(
        "ALPACA_BASE_URL", ALPACA_PAPER_BASE_URL_DEFAULT
    ).strip().rstrip("/")
    if base_url != ALPACA_PAPER_BASE_URL_DEFAULT:
        raise RuntimeError(
            f"ALPACA_BASE_URL must be {ALPACA_PAPER_BASE_URL_DEFAULT} (paper only)"
        )
    ...
    return Config(
        ...
        alpaca_api_key=_clean("ALPACA_API_KEY"),
        alpaca_secret_key=_clean("ALPACA_SECRET_KEY"),
