...
async def run_pipeline(
    ...
    for instrument in cfg.instruments:
        ...
        # None when no override has been pushed for this instrument.
        ...
        # Booster B: Multi-timeframe trend confluence — fetch H4 alongside H1
        # so filters.mtf_trend_filter can require trend agreement across both.
        mtf_bars = None
        try:
            h4 = await asyncio.to_thread(
                fetcher.fetch_candles, instrument, "H4", 32
            )
            mtf_bars = {"H4": h4, "H1": hist}
        except Exception as e:
        ...
        # Layer 4b: Pre-execution filters (multi-TF trend, indicator confluence, session)
        # These run AFTER synth so we keep Tauric's reasoning + cycle log entries,
        ...
        # R:R + ATR multiplier — overridden per-symbol via dashboard Auto-Tune.
        rr = (
            (rr_base + 1.0) if verdict.confidence >= 9
            else (rr_base + 0.5) if verdict.confidence >= 8
            else rr_base
        )
        sl_pips = adaptive_sl_pips(hist, instrument, multiplier=atr_mult) or 10.0
        # Conviction × volatility position sizing (Gaps #3 + #4):
        # bigger when Tauric is confident AND markets are calm; half size in chop.
        sized = size_position(decision.units, verdict.confidence, kronos_sig.vol_amp)
        if sized.final_units <= 0:
            log.info(
                "position_sizer_vetoed",
                instrument=instrument,
                reason=sized.reason,
            )
            continue
...
async def main_async() -> None:
    ...
    # Endpoints: /health, /cycles, /trades. Gated by X-Status-Token header.
